...
namespace ControleFacil.Api.Damain.Repository.Classes
{
    public class AreceberRepository : IAreceberRepository
    {
        ...
        {
             Areceber? entidadeBanco = await  _contexto.Areceber.Where(u => u.Id == entidade.Id).FirstOrDefaultAsync();
        ...
        {
            return await _contexto.Areceber.AsNoTracking().OrderBy(u => u.Id).ToListAsync();
        }

        public async Task<Areceber> Obter(long id)
        {
            return await _contexto.Areceber.AsNoTracking().Where(n => n.Id == id).FirstOrDefaultAsync();
        }
