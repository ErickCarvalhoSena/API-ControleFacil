...
namespace ControleFacil.Api.Damain.Repository.Classes
{
    public class ApagarRepository : IApagarRepository
    {
        ...
        {
             Apagar? entidadeBanco = await  _contexto.Apagar.Where(u => u.Id == entidade.Id).FirstOrDefaultAsync();
        ...
        {
            return await _contexto.Apagar.AsNoTracking().OrderBy(u => u.Id).ToListAsync();
        }

        public async Task<Apagar> Obter(long id)
        {
            return await _contexto.Apagar.AsNoTracking().Where(n => n.Id == id).FirstOrDefaultAsync();
        }
