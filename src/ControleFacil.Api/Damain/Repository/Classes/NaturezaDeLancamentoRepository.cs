...
namespace ControleFacil.Api.Damain.Repository.Classes
{
    public class NaturezaDeLancamentoRepository : INaturezaDeLancamentoRepository
    {
        ...
        {
             NaturezaDeLancamento? entidadeBanco = await  _contexto.NaturezaDeLancamento.Where(u => u.Id == entidade.Id).FirstOrDefaultAsync();
        ...
        {
            return await _contexto.NaturezaDeLancamento.AsNoTracking().OrderBy(u => u.Id).ToListAsync();
        }

        public async Task<NaturezaDeLancamento> Obter(long id)
        {
            return await _contexto.NaturezaDeLancamento.AsNoTracking().Where(n => n.Id == id).FirstOrDefaultAsync();
        }
